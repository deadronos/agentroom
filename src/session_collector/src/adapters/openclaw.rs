...
impl OpenClawAdapter {
    ...
    fn log_dir() -> PathBuf {
        Self::home_dir().join(".openclaw").join("logs")
    }

    fn projects_dir() -> PathBuf {
        Self::home_dir().join(".openclaw").join("projects")
    }
...
impl SessionAdapter for OpenClawAdapter {
        ...
        for dir in [Self::log_dir(), Self::projects_dir()] {
            ...
            for path in paths {
                ...
                if let Ok(stat) = std::fs::metadata(&path) {
                    ...
                    let session_id = format!("openclaw:{}", Self::parse_session_id(&path));
    ...
    fn session_detail(&self, session_id: &str) -> Option<ActiveSession> {
        let path_str = session_id.strip_prefix("openclaw:")?;
